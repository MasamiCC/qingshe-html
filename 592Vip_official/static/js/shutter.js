...
;(function ($, window, document, undefined) {
  ...
  Shutter.prototype = {
    ...
    toggleMove: function (command, index) {
     ...
     this.moveSwitch(random(0, 10), command, index);
    ...
    inital: function (elem) {
      ...
      this.$paginationSpan = this.$shutter.find('.pagination span');
      ...
      this.$paginationSpan.bind('click',function(){
      	if (self.b_stop) {
          self.b_stop = false;
          var index = $(this).index();
          self.toggleMove(false , index);
        }
      })
      ...
      if (this.opts.fullPage) {
        $(window).resize(function () {
          setTimeout(function () {
          	//最小宽度1280
          	if( $(this).width() <=1320){
          		self.$shutter.css({width: 1320, height: $(this).height()});
          	}else{
    ...
    gridWhole: function (index, showNum) {
      ...
      $createElem.find('.change_img').css({
        display: 'block',
        width: this.shutterW,
        height: this.shutterH
      });
    ...
    gridAccordion: function (index) {
      ...
      $targetElem.css('zIndex', 19);
      $curElem.find('.change_img').fadeOut();
