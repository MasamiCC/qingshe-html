...
Showbo.Msg={  
    ...
    InitMsg:function(width){  
      ...
      html='<div class="div_small"><div class="top"><div class="right"><div class="title" id="dvMsgTitle"></div></div></div>'+  
...
//    },  
    ...
    deliveryConfirm:function(msg,fn,title){
    	this.show({buttons:{yes:'确认',no:'取消'},msg:msg,title:title,fn:fn}); 
    },
    prompt:function(labelWord,defaultValue,txtId,title,fn){  
      if(!labelWord)labelWord='请输入：';  
      if(!defaultValue)defaultValue="";  
      if(!txtId)txtId="msg_txtInput";
      if(!title)title="输入提示";
      this.show({title:title,msg:labelWord+'<input type="password" maxlength="6" id="'+txtId+'" class="input paycode" style="width:200px" value=""/>',buttons:{yes:'确认',no:'取消'},fn:fn});  
    }, 
     
    shelvesConfirm:function(msg,fn,fn2){
    	this.show({title:'提示',msg:msg,buttons:{yes:'继续上架',cancel:'返回'},fn:fn,fn2:fn2}); 
    },
    changeConfirm:function(msg,fn,fn2){
    	this.show({title:'提示',msg:msg,buttons:{yes:'保存',cancel:'不保存'},fn:fn,fn2:fn2}); 
    },
    redsSendConfirm:function(data,fn,fn2){
      this.show({title:'',msg:data.msg,buttons:data.buttons,fn:fn,fn2:fn2}); 
    },
    ...
    show:function(cfg){  
      ...
      if(cfg.wait){  
        ...
        if(!cfg.buttons||(!cfg.buttons.yes&&!cfg.buttons.no&&!cfg.buttons.next&&!cfg.buttons.confirm)){  
          cfg.buttons={yes:'确定'};  
        }  
        ...
        if(cfg.buttons.yes){  
          ...
          if(cfg.buttons.no || cfg.buttons.cancel)this.dvBtns.appendChild(document.createTextNode('　'));  
        }  
        
        if(cfg.buttons.cancel){
        	 this.dvBtns.appendChild(this.createBtn('cancel',cfg.buttons.cancel,cfg.fn2));
        }
    ...
    onResize:function(isResize){  
       ...
       Showbo.Msg.lightBox.style.position="fixed";
       Showbo.Msg.lightBox.style.overflow="hidden";
...
function showMessage(callBack,msg){
	Showbo.Msg.show(
		{title:'提醒'
		,msg:msg
		,buttons:{yes:'确定'}
