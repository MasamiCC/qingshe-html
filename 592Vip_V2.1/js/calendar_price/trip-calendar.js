...
Y.TripCalendar = Y.extend(Calendar, Y.Base, {
    ...
    _getMarked:function(v){
    	//console.log(this.get('datas')[v])
    	console.log(this._getDateStatus(v));
    	var marker='';
    	if(!this._getDateStatus(v)){
                    ...
                    toHTML(Calendar.DAY_TEMPLATE,
                        {
                            ...
                            'marked':toHTML(Calendar.MARKER_DATE_TEMPLATE,{
                            	'price':this._getMarked(date)?"￥"+this._getMarked(date)['price']:'',
                            }),
                        }
                    )
            }
...
},
{
    ...
    DAY_TEMPLATE: '<td data-date="{date}" class="{disabled}">' +'{marked}'+
    ...
    MARKER_DATE_TEMPLATE: '<span class="marker">{price}</span>',
    /**
     * 日期名字
     *
     * @property DATENAME
