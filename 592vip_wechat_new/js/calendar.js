...
( function( root, factory ) {
    ...
    var tpl = [
        '<div class="cal-wrap mui-slider-item mui-control-content">',
            '<h2>{%date%}</h2>',
            '{%week%}',
            '<ul class="day">',
                '{%day%}',
    ...
    $.extend( calendar.prototype, {
        ...
        _initDate: function() {
            ...
            this.markData=config.markData;
            this.isShowMarker=config.isShowMarker;
        },
        ...
        _getDayList: function( year, month ) {
            ...
            for ( var i = 0; i < days; i++ ) {
                ...
                if (config.isShowMarker) {
                    var  markData=config.markData;

                    for (var j = 0; j < markData.length; j++) {
                        var date=markData[j]['date'];
                        var price=markData[j]['price'];
						var pd=markData[j]['pd'];
                        if (dateNum==dateToNum( date )) {
                            tmpDayDataArr[ i ][ 'class' ] = jrClassName;
                            if(price){
                            	tmpDayDataArr[ i ][ 'name' ] = price;
                            }
                            
                            if (pd) {
                            	tmpDayDataArr[ i ][ 'title' ] = pd;
                            }
...
//                      console.log(date);
                ...
                if ( config.selectDate ) {
                    ...
                    }else if(dateNum < dateToNum( me.selectDate )){
                    	tmpDayDataArr[ i ][ 'class' ] += ' ed';
                    }
                }
            }
            ...
            for ( var i = 0, len = tmpDayDataArr.length; i < len; i++ ) {
                ...
                if (item.title) {
                	tmpDayTplArr.push(
	                    '<li class="' + item.class + '" data-date="' + item.date + '" data-pd="'+item.title+'">' +
