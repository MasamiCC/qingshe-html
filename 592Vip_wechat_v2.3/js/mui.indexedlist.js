...
(function($, window, document) {
	...
	var IndexedList = $.IndexedList = $.Class.extend({
		...
		caleLayout: function() {
			...
			self.el.bar.style.height = self.el.bar.offsetHeight+'px';
			self.el.bar.style.top=(self.box.offsetHeight-self.el.bar.offsetHeight)/2+'px'
		...
		bindBarEvent: function() {
			...
			var findStart = function(event) {
				...
				if (pointElement) {
					...
					if (group.length==1||group=='区县'||group=='定位'||group=='热门' ) {
		...
		search: function(keyword) {
			...
			liArray.forEach(function(item) {
				...
				if (item.classList.contains($.className('indexed-list-group'))) {
					...
					if (keyword && text.indexOf(keyword) < 0 &&
						...
						selectorBuffer.push(classSelector('indexed-list-inner .mui-table-view> li') + ':nth-child(' + (currentIndex + 1) + ')');
						itemCount++;
					}
			...
			selBuffer.forEach(function(item){
				console.log(item)
			})
		},
