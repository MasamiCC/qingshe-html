...
var goodsData={
	...
	result: [
	        ...
	        id: 28,
	        name: "小房间",
	        img: "shop/10024/28/7f826f55-7100-4b0f-9cb3-a49b788739e0.png",
...
$(function(){
	...
	addCartClick();
	//loadGoodsData(goodsData.result);
...
function loadCartData(){
	...
	for (var i in chartData) {
		...
		var price=parseFloat(chartData[i]['price']*chartData[i]['num']).toFixed(2) ;
			html+=	'</div>'
	...
	$(".cart_lists ul").append('<div class="cart_list"><h4>入住天数 <span style="float:right;font-weight:normal">*2</span></h4></div>')
...
function loadGoodsData(data){
	console.log(data)
	$('#cat_list').empty();
	var html='<div class="cat_item" dir="ltr" data-id="1" id="cat_item1">'
        	+'<div class="shopping_cat_title">'
        	+	'选择房型'
        	+'</div>'
        	+'<div class="shopping_cat_goods">'
	for (var i in data) {
		html+='<div class="shopping_cat_item">'
    		+	'<div class="shopping_cat_img">'
    		+		'<img src="../imgs/comment_pic1.png">'
    		+	'</div>'
    		...
    		+			data[i]['attribute1']+'<br/>'+data[i]['attribute2']
    		+		'</div>'
...
//  		+			'</div>'
    		...
    		+				'￥'+data[i]['price']
    		+				'<button data-goodsid="'+data[i]['id']+'" data-price="'+data[i]['price']+'" class="addToCart" data-name="'+data[i]['name']+'" >加入购物车</button>'
    		+			'</div>'
    		+		'</div>'
    		+	'</div>'
    		+'</div>';
